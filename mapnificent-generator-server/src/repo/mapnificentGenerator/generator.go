...
import (
	...
	"fmt"
	"io"
	// add ioutil for ReadFile functionality
	// "io/ioutil"
	"log"
	"os"
	"path/filepath"
...
var (
	// pathsString is used in Generate()
	...
	pathsString string = "tmp/gtfz.zip"
	// outputFile is used in Generate()
	// outputFile  = flag.String("o", "", "Output file")
	outputFile string = "tmp/gtfs.bin"
	// shouldLog is used in Generate()
	// shouldLog   = flag.Bool("v", false, "Log to Stdout/err")
	shouldLog bool = false
	// extraInfo is used in GetNetwork(), GetOrCreateMapnificentStop() and Generate()
	//extraInfo   = flag.Bool("e", false, "Add extra info to output")
	extraInfo bool = false
	// needHelp is used in Generate()
	//needHelp    = flag.Bool("h", false, "Displays this help message...")
	needHelp bool = false
	// feeds is used in
	// gtfs is the package github.com/mapnificent/gogtfs
	// gtfs.Feed is the exportable function from the gtfs package
	/* gtfs.Feed structure:
	type Feed struct {
...
func GetNetwork(feeds map[string]*gtfs.Feed, extraInfo bool) *mapnificent.MapnificentNetwork {
	...
	for path, feed := range feeds {
		log.Println("GetNetwork loop", path)
		...
		for _, li := range lineMap {
			...
			for _, stoptime := range trip.StopTimes {
				stopIndex := GetOrCreateMapnificentStop(feeds, path, stoptime.Stop, network, stationMap, extraInfo)
	...
	if !ok {
		...
		for localPath, feed := range feeds {
			...
			for _, nearbyStopDistance := range nearbyStopDistances {
				...
				nearbyStopName := fmt.Sprintf("%s_%s", localPath, nearbyStop.Id)
				if nearbyStopName == stationName {
					// same stop
					continue
				}
...
func Generate(pathsString string, outputFile string) {
	/*
	flag.Parse()
